...
public static class CatmullRom
{
    ...
    {
        return quaternion.LookRotation(math.normalize(GetTangent(p0, p1, p2, p3, time)), new float3(0.0f, 1.0f, 0.0f));
    }
    ...
    public static float3 GetConcavity(float3 p0, float3 p1, float3 p2, float3 p3, float time)
    {
        ...
        float3 deriv2 = 0.5f * (2.0f * (2.0f * p0 - 5.0f * p1 + 4.0f * p2 - p3) + 6 * (-p0 + 3 * p1 - 3 * p2 + p3) * t);
