...
public class DollyTrackAutoSpeed : MonoBehaviour
{
    ...
    [Range(0, 1)]
    public float    m_MaxTurnSpeedModifier = 0.25f;
    [Range(0, 1)]
    private float   m_TargetTurnSpeed = 1.0f;
    private float   m_CurrentTurnSpeed = 1.0f;

    public float    m_ClimbLookAheadPathDistance = 1.0f;
    public float    m_ClimbModifier = 1.0f;
    [Range(0, 1)]
    private float   m_ClimbDecelleration = 0.0f;
    ...
    {
        float3 currDollyForward = math.mul(m_Path.EvaluateOrientationAtUnit(m_DollyCart.m_Position, m_DollyCart.m_PositionUnits), c_FORWARD);
        ...
        if (m_CurrentFallSpeed != m_TargetFallSpeed)
            m_CurrentFallSpeed = m_CurrentFallSpeed + (m_CurrentFallSpeed < m_TargetFallSpeed ? m_FallAcceleration : -m_FallDecelleration);
    ...
    {
        float3 currDollyForward = math.mul(m_Path.EvaluateOrientationAtUnit(m_DollyCart.m_Position, m_DollyCart.m_PositionUnits), c_FORWARD);
        float3 nextDollyForward = math.mul(m_Path.EvaluateOrientationAtUnit(m_DollyCart.m_Position + m_ClimbLookAheadPathDistance, m_DollyCart.m_PositionUnits), c_FORWARD);
