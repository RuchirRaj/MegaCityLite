...
// Very simple script to alter the HD Renderpipeline Settings on first run based off the quality settings.
    ...
    private void ApplyRenderSettings()
    {
        ...
        if (hdCam)
        {
            ...
            if (m_RenderSettings.TryGetValue(qualityName, out currentSettings))
            {
                ...
                hdCam.customRenderingSettings = true; // Apply our changes above otherwise we use the default settings.
                ...
                hdCam.renderingPathCustomFrameSettingsOverrideMask.mask[(uint)FrameSettingsField.ContactShadows] = true;
                hdCam.renderingPathCustomFrameSettingsOverrideMask.mask[(uint)FrameSettingsField.Volumetrics] = true;
                hdCam.renderingPathCustomFrameSettingsOverrideMask.mask[(uint)FrameSettingsField.ReprojectionForVolumetrics] = true;
                ...
                hdCam.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.ContactShadows, currentSettings.contactShadows);
                hdCam.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.Volumetrics, currentSettings.volumetrics);
                hdCam.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.ReprojectionForVolumetrics, currentSettings.volumetrics);
            }
            else
            {
