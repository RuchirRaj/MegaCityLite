...
public class DelayLineDopplerHack : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < m_SourceData.Length; i++)
        {
            ...
            s.m_Attenuation[0] = attenuation * (0.65f + m_PanAmount * 0.45f * Mathf.Clamp(leftDir * Mathf.Abs(leftDir) / (len + 0.001f), -1.0f, 1.0f));
            s.m_Attenuation[1] = attenuation * (0.65f + m_PanAmount * 0.45f * Mathf.Clamp(rightDir * Mathf.Abs(rightDir) / (len + 0.001f), -1.0f, 1.0f));
        ...
        for (int n = 0; n < data.Length; n += numChannels)
        {
            ...
            for (int i = 0; i < m_SourceData.Length; i++)
            {
                ...
                for (int c = 0; c < numChannels; c++)
                {
                    ...
                    s.m_InterpolatedAttenuation[c] += (s.m_Attenuation[c] - s.m_InterpolatedAttenuation[c]) * envelopeSpeed;
